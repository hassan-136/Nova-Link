...
PORT = 8888         # port for DeltaCore

clients = []

FILES_DIR = os.path.join(os.path.dirname(__file__), "deltacore_files")  # separate folder for DeltaCore files

def handle_client(conn, addr):
    print(f"[NEW CONNECTION] {addr} connected to DeltaCore server.")
    conn.sendall(b"Welcome to DeltaCore VPN server!\n")
...
def start_server():
    ...
    server.listen()
    print(f"[LISTENING] DeltaCore server listening on {HOST}:{PORT}")
