...
PORT = 7777         # port for GammaX

clients = []

FILES_DIR = os.path.join(os.path.dirname(__file__), "gammx_files")  # separate folder for GammaX files

def handle_client(conn, addr):
    print(f"[NEW CONNECTION] {addr} connected to GammaX server.")
    conn.sendall(b"Welcome to GammaX VPN server!\n")
...
def start_server():
    ...
    server.listen()
    print(f"[LISTENING] GammaX server listening on {HOST}:{PORT}")
