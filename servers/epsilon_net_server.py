...
PORT = 9999         # port for EpsilonNet

clients = []

FILES_DIR = os.path.join(os.path.dirname(__file__), "epsilonnet_files")  # separate folder for EpsilonNet files

def handle_client(conn, addr):
    print(f"[NEW CONNECTION] {addr} connected to EpsilonNet server.")
    conn.sendall(b"Welcome to EpsilonNet VPN server!\n")
...
def start_server():
    ...
    server.listen()
    print(f"[LISTENING] EpsilonNet server listening on {HOST}:{PORT}")
